...
int IO_MqttParse_IO_RTC_SET_MODE(const char *pCmd, const void *pJson_Obj, void *pParam, void *Json_CmdInfo_Out) {
  ...
  if (j_args != nullptr) {
    ...
    MAINLOG(0, "[__%s__] %d: [%d][%d][%d] [%d][%d][%d] [%s]\n", __func__, __LINE__, ios_rtc.local_time.tm_year, ios_rtc.local_time.tm_mon, ios_rtc.local_time.tm_mday, ios_rtc.local_time.tm_hour, ios_rtc.local_time.tm_min, ios_rtc.local_time.tm_sec, ios_rtc.local_time.tm_zone);
  }
  memset((char *)ios_CmdInfo, '\0', sizeof((char *)ios_CmdInfo));
  strcpy((char *)ios_CmdInfo, json_object_get_string(j_args));
  return 0;
...
int IO_MqttParse_IO_AILIGHTING_SET_PARAM(const char *pCmd, const void *pJson_Obj, void *pParam, void *Json_CmdInfo_Out) {
  ...
  if (j_args != nullptr) {
    ...
    if (j_param) {
      ...
      xlog("ios_ailighting.intBrightness:%d", ios_ailighting.intBrightness);
    }

    int led1, led2, led3, led4, value;
    if (sscanf(ios_ailighting.strlightSource.c_str(), "[%d,%d,%d,%d]", &led1, &led2, &led3, &led4) >= 4) {
      xlog("sscanf >= 4");
      ios_ailighting.channel = value = (char)(led1 | led2 << 1 | led3 << 2 | led4 << 3) & 0x0F;
