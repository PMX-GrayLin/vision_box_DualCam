...
static const char *device = "/dev/spidev0.0"; //?$)AD/?????$)AP>???pi?$)AVj$)AK!???
static uint8_t mode = 0; /* SPI???????$)A5d??$)A7%?$)Av,??CPOL????PHA????*/
static uint8_t bits = 8; /* 8bit?????SB first??/
//static uint32_t speed = 2*1000*1000;/* $)Av,???????? */
static uint32_t speed = 100000;/* $)Av,???????? */
static uint16_t delay = 100000;
